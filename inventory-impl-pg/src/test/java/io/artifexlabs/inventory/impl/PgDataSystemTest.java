...
/**
 * The Postgres backend against the SAME parity kit the in-memory twin runs, which is the point: the dictionary
 * encoding, the two byte-hashes, and the transactional replace are invisible through the interface, so every assertion
 * is written once and proven twice.
 *
...
public class PgDataSystemTest extends DataSystemTck {
  ...
  protected Backends backends() throws Exception {
    ...
    return new Backends(items, new PgDataSystem(pool, items, "tester@example.com"), new PgAudit(pool));
  }
}
