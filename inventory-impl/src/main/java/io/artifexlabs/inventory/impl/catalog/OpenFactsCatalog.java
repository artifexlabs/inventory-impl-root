...
/**
 * The Open Food Facts family (ODbL open data, keyless REST): flavors are
 * tried in order — products, food, beauty, pet food — and the first hit
 * wins. Every failure mode (HTTP error, timeout, unparseable body) degrades
 * to a miss: lookup is prefill, never a dependency.
 *
 * The {@code source=} tag carried on created items links the flavor's stable
 * product page — our ODbL attribution.
 */
public class OpenFactsCatalog implements UpcCatalog {
  ...
  public final static List<String> DEFAULT_BASES = List.of(
  ...
  private CompletionStage<Optional<CatalogEntry>> tryFlavor(String gtin13, int index) {
    ...
    HttpRequest request = HttpRequest.newBuilder()
        .uri(URI.create(base + "/api/v2/product/" + gtin13 + ".json?fields=" + FIELDS))
    ...
    return this.http.sendAsync(request, HttpResponse.BodyHandlers.ofString())
        .<Optional<CatalogEntry>>thenApply(response -> response.statusCode() == 200
            ? parse(gtin13, base, response.body())
            : Optional.empty())
        .exceptionally(e -> {
  ...
  private static Optional<CatalogEntry> parse(String gtin13, String base, String body) {
    try {
      ...
      return Optional.of(new CatalogEntry(gtin13, name,
          blankToNull(product.getString("brands")),
          blankToNull(product.getString("generic_name")),
          leafCategory(product.getString("categories")),
          weightGrams(product),
          blankToNull(product.getString("image_url")),
          base + "/product/" + gtin13,
