...
public class UsersVerticle extends ServiceVerticle {
  ...
  public UsersVerticle(BusGuard guard, UserStore users, AuditSink audit) {
    super(BusActions.addressOf(BusActions.USERS_LIST), guard);
    on(BusActions.USERS_LIST, env -> users.list()
        .thenApply(list -> new JsonArray(list.stream().map(UserFactory::serialize).toList())));
    on(BusActions.USERS_CREATE, env -> {
    ...
    on(BusActions.USERS_DELETE, env -> {
    ...
    on(BusActions.USERS_SET_ADMIN, env -> {
