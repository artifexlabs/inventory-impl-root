...
/**
 * Regions operations against the backing store (PLAN.md Phase 21, ask 2). These are
...
final class RegionsStorage {
  ...
  static void register(StorageVerticle.Registrar reg, RegionSystem regions) {
    ...
    reg.on(BusActions.REGIONS_DELETE, env -> regions.actingAs(env.principal()).deleteRegion(Envelopes.requireTarget(env)).thenApply(ok -> {
      if (!ok)
